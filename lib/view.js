define(['data', 'date'], function (data, date) {
    ...
    var app = {
             ...
             * @param {number} value
             */
            setAccountBalance: function (value) {
                app.level = value;
            ...
            getRow: function (balance, amount, name, icon) {
                var $row = $('<div>').addClass('row-fluid'),
                    $balance = $('<div>').addClass('span2').text(app.numberWithCommas(Math.round(balance)) || ''),
             ...
             * @param  {[type]} key
             * @return {object|string} jquery or ''
             */
                    ...
                    if (date.getRetroKey() <= key) {
                        ...
                        _.each(calendar[key], function (id) {
                            ...
                            if (date.getNowKey() <= key) {
                                app.setAccountBalance(app.balance + service.amount);
                                content.append($('<p>').append(app.getRow(app.balance, service.amount, service.name, service.icon)));
                            } else {
            ...
            render: function () {
                //add current
                $($.find('#current')).text('current: ' + Math.round(app.balance));
                ...
                $($.find('#min')).text('min: ' + Math.round(app.balancemin || app.balance));
            }
        };
